...
class OD2Score(torch.nn.Module):
    ...
    def __init__(self, target_bbox: np.ndarray, device: torch.device, conf_thresh: float = 0.01, iou_thresh: float = 0.5):
...
def calculate_shap(model, super_pixler: SuperPixler, bbox_target: torch.Tensor, nsamples: int = 80, batch_size: int = 256, half: bool = False) -> np.ndarray:
    ...
    patch_scorer = torch.nn.Sequential(
            ...
            CastNumpy(to="torch", device=device, half=half),  # (n_outputs, H, W, C) of float torch
            Permute(0, 3, 1, 2),  # (n_outputs, C, H, W) of float torch
            model,  # (n_outputs, num_detections, 6) of float torch
            OD2Score(bbox_target, device, model.iou),  # (n_outputs, n_detections, 6) -> (n_outputs) of float torch,
...
def plot_shap_explenations(img: np.ndarray, shap_values: np.ndarray, segments: np.ndarray, blend_alpha: float = 0.5):
    assert img.ndim == 3
